...
*** 단, 4번째 입력칸인 Stream_User 입력시 IDCS user로 진행하는 경우에는 입력하는 user 이름을 apackrsct01/oracleidentitycloudservice/<mail_id> 로 변경해야 합니다.
(user name을 stream > kafka connection settings > user name 복사한 후에 apackrsct01 다음에 oracleidentitycloudservice 를 추가로 적어줘야 합니다) 
